...
const width = canvas.offsetWidth || 800;  // ZERO because parent is hidden!
const height = canvas.offsetHeight || 500; // ZERO because parent is hidden!
...
Show viewer before Three.js init:
...
Create camera with correct dimensions:
...
| Item | Before | After |
|------|--------|-------|
| Viewer visibility | Hidden (display:none) | Visible (removed class) |
