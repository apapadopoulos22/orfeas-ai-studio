#!/usr/bin/env python3
"""Test STL file download and verify size"""
import requests
import time
import os
...
DOWNLOAD_URL = f"{API_BASE}/api/download/{JOB_ID}/model_{JOB_ID}.stl"
OUTPUT_FILE = f"test_download_{JOB_ID}.stl"
...
try:
    ...
    if content_length and actual_size != int(content_length):
        ...
        print(f"[TEST]    Difference: {int(content_length) - actual_size:,} bytes")
    else:
