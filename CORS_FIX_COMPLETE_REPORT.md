...
2. **Explicit headers** - Uses dictionary assignment for reliability
...
Endpoint: `http://127.0.0.1:5000/api/models-info`
Method: OPTIONS
...
- [CORS] Preflight OPTIONS request handled (from origin: none)
...
1. Browser Request (file:// origin)
2. OPTIONS Preflight (automatic)
3. @app.before_request handler
4. Response: 204 + CORS headers
5. Browser checks: Access-Control-Allow-Origin: * ✅
6. Browser sends actual request
7. Route handler processes request
8. Flask-CORS adds headers to response
9. Response: 200 + body + CORS headers
...
3. Verify backend logs have `[CORS]` entries
...
Invoke-WebRequest -Uri 'https://your-ngrok-url/api/health' `
    -UseBasicParsing
```
...
**Fix**: Verify line 840 in `backend/main.py` has:
...
| **Preflight Caching** | -50ms | Max-Age: 86400 means 24h cache |
| **Memory** | None | CORS headers are minimal (~200 bytes per response) |
| **GPU Impact** | None | All CORS handling is CPU-side |
...
1. **Enable Credentials** (if needed):

    ```python
    allow_credentials=True
    ```

1. **Monitor Preflight Calls**:
...
**The CORS issue is resolved. Frontend can now communicate with backend
