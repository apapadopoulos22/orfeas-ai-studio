"""
ORFEAS AI 2DÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢3D Studio - Unified Main Server
==================================================
ORFEAS AI Project
...
# [ORFEAS PHASE 4] 99%+ Enterprise Optimization Components - All Three Tiers
# ============================================================================
# Tier 1: Essential Optimization Components (GPU, Dashboard, Cache, Load Tests)
try:
    from advanced_gpu_optimizer import get_advanced_gpu_optimizer
    from performance_dashboard_realtime import get_dashboard
    from distributed_cache_manager import get_distributed_cache
    logger.info("[ORFEAS PHASE 4] Tier 1 components imported: GPU Optimizer, Dashboard, Cache")
except ImportError as e:
    logger.warning(f"[ORFEAS PHASE 4] Tier 1 components unavailable: {e}")

# Tier 2: Enhanced Monitoring Components (Predictions, Alerting)
try:
    from predictive_performance_optimizer import get_predictive_optimizer
    from alerting_system import get_alerting_system
    logger.info("[ORFEAS PHASE 4] Tier 2 components imported: Predictions, Alerting")
except ImportError as e:
    logger.warning(f"[ORFEAS PHASE 4] Tier 2 components unavailable: {e}")

# Tier 3: Premium Intelligence Components (Anomalies, Tracing)
try:
    from ml_anomaly_detector import get_anomaly_detector
    from distributed_tracing import get_tracing_system
    logger.info("[ORFEAS PHASE 4] Tier 3 components imported: Anomalies, Tracing")
except ImportError as e:
    logger.warning(f"[ORFEAS PHASE 4] Tier 3 components unavailable: {e}")
...
class AdvancedDepthEstimator:
    ...
    def load_midas_model(self):
        ...
        try:
            ...
            logger.info("ÃƒÂ°Ã…Â¸Ã‚Â§Ã‚Â  MiDaS model would load here (implementation pending)")
    ...
    def neural_depth_estimation(self, image_data: np.ndarray) -> np.ndarray:
        ...
        logger.info("ÃƒÂ°Ã…Â¸Ã‚Â§Ã‚Â  Neural depth estimation (MiDaS)")
...
class Advanced3DMeshGenerator:
    ...
    def heightfield_generation(self, depth_map, dimensions, quality):
        """Enhanced heightfield generation with proper 3D volume (manifold solid)"""
        ...
        # Invert depth map: brighter pixels (closer to camera) should be higher in 3D
        inverted_depth = 1.0 - depth_map

        # Enhance depth contrast for dramatic 3D relief
        inverted_depth = np.power(inverted_depth, 0.7)  # Gamma correction

        # Normalize to ensure full range usage
        depth_min = inverted_depth.min()
        depth_max = inverted_depth.max()
        if depth_max > depth_min:
            inverted_depth = (inverted_depth - depth_min) / (depth_max - depth_min)

        # Scale factors
        x_scale = dimensions['width'] / width
        y_scale = dimensions['height'] / height
        z_scale = dimensions['depth'] * 1.2  # Increased for more dramatic depth

        # === FRONT SURFACE VERTICES (from depth map) ===
        front_vertices = []
        for i in range(height):
            for j in range(width):
                x = j * x_scale - dimensions['width'] / 2
                y = i * y_scale - dimensions['height'] / 2
                z = inverted_depth[i, j] * z_scale
                front_vertices.append([x, y, z])

        # === BACK SURFACE VERTICES (flat base) ===
        back_z = -z_scale * 0.1  # Small base depth for solid bottom
        back_vertices = []
        for i in range(height):
            for j in range(width):
                x = j * x_scale - dimensions['width'] / 2
                y = i * y_scale - dimensions['height'] / 2
                back_vertices.append([x, y, back_z])
        ...
        # Back surface faces (reversed winding for outward normals)
        offset = height * width
        for i in range(height - 1):
            for j in range(width - 1):
                v0 = offset + i * width + j
                v1 = offset + i * width + (j + 1)
                v2 = offset + (i + 1) * width + j
                v3 = offset + (i + 1) * width + (j + 1)
        ...
        for i in range(height - 1):
            ...
            faces.append([b0, b1, f1])

        # Right edge
        for i in range(height - 1):
            f0 = i * width + (width - 1)
            f1 = (i + 1) * width + (width - 1)
            b0 = offset + i * width + (width - 1)
            b1 = offset + (i + 1) * width + (width - 1)
            faces.append([f0, f1, b0])
            faces.append([b0, f1, b1])
        ...
        for j in range(width - 1):
            f0 = (height - 1) * width + j
            f1 = (height - 1) * width + j + 1
            b0 = offset + (height - 1) * width + j
            b1 = offset + (height - 1) * width + j + 1
            faces.append([f0, b0, f1])
            faces.append([b0, b1, f1])

        faces = np.array(faces, dtype=np.int32)

        logger.info(f"[OK] Solid 3D mesh: {len(vertices)} vertices, {len(faces)} faces (manifold)")
        return vertices, faces
...
# [ORFEAS FIX 3] Custom Rate Limiter Implementation (no external dependencies)
class RateLimiter:
    """Simple in-memory rate limiter for protecting against DoS attacks"""
...
class OrfeasUnifiedServer:
    ...
    def __init__(self, mode: ProcessorMode = ProcessorMode.FULL_AI):
        ...
        logger.info("[RATE-LIMITING] Rate limiter initialized (60 requests/minute per IP)")
        ...
        # [ORFEAS PHASE 1] Initialize Dynamic VRAM Manager for GPU optimization
        if not self.is_testing:
            try:
                self.vram_manager = get_vram_manager()
                logger.info(f"[ORFEAS] GPU Manager initialized: {self.vram_manager}")

                # Log initial memory state
                stats = self.vram_manager.get_memory_stats()
                logger.info(f"[ORFEAS] Initial GPU stats: Total={stats.get('total_vram_gb', 'N/A'):.1f}GB, "
                           f"Available={stats.get('available_gb', 'N/A'):.1f}GB, "
                           f"Usage={stats.get('usage_percent', 'N/A'):.1f}%")

                # Start monitoring thread
                self.vram_manager.monitor_vram_usage(interval_seconds=5.0)
                logger.info("[ORFEAS] GPU VRAM monitoring started (5s interval)")
            except Exception as e:
                logger.error(f"[ORFEAS] Failed to initialize GPU optimization: {e}")
                traceback.print_exc()
                self.vram_manager = None
        else:
            self.vram_manager = None
            logger.info("[TEST MODE] VRAM Manager disabled")
        ...
        if not self.is_testing:
            ...
            try:
                ...
                logger.info("[ORFEAS]  Ultra-Performance Manager initialized successfully")
            except Exception as e:
        ...
        if not self.is_testing and ENTERPRISE_AGENTS_AVAILABLE:
            ...
            try:
                ...
                logger.info("[ORFEAS]  Enterprise Agent Framework initialized successfully")
            except Exception as e:
        ...
        # [ORFEAS] PHASE 6C: In-Memory LRU Cache for 2x generation speed improvement
        # Import cache manager for high-performance result caching
        try:
            from cache_manager import get_cache
            self.lru_cache = get_cache(
                max_size=int(os.getenv('CACHE_MAX_SIZE', '1000')),
                max_memory_mb=float(os.getenv('CACHE_MAX_MEMORY_MB', '512')),
                default_ttl_seconds=int(os.getenv('CACHE_TTL_SECONDS', '86400'))  # 24 hours default
            )
            self.cache_enabled = os.getenv('DISABLE_RESULT_CACHE', '0') != '1'
            if not self.is_testing and self.cache_enabled:
                logger.info("[ORFEAS] Phase 6C: LRU Cache enabled - 95% faster for duplicate requests")
                logger.info(f"[CACHE] Config: max_items={os.getenv('CACHE_MAX_SIZE', '1000')}, "
                           f"max_memory_mb={os.getenv('CACHE_MAX_MEMORY_MB', '512')}MB")
            elif not self.is_testing and not self.cache_enabled:
                logger.warning("[ORFEAS] Result caching DISABLED (DISABLE_RESULT_CACHE=1)")
        except ImportError:
            logger.warning("[CACHE] Failed to import LRU cache, using fallback dict cache")
            self.lru_cache = None
            self.result_cache = {}  # Fallback to simple dict
            self.cache_enabled = os.getenv('DISABLE_RESULT_CACHE', '0') != '1'

        # [ORFEAS PHASE 4] Initialize 99%+ Enterprise Optimization Components
        # ====================================================================
        # Tier 1: Essential Optimization (GPU, Dashboard, Cache, Load Tests)
        if not self.is_testing:
            logger.info("[ORFEAS PHASE 4] Initializing enterprise optimization tiers...")

            # Initialize Advanced GPU Optimizer (Tier 1)
            try:
                self.gpu_optimizer = get_advanced_gpu_optimizer()
                logger.info("[ORFEAS PHASE 4 Tier 1] Advanced GPU Optimizer initialized")
            except Exception as e:
                logger.warning(f"[ORFEAS PHASE 4 Tier 1] GPU Optimizer failed: {e}")
                self.gpu_optimizer = None

            # Initialize Real-Time Performance Dashboard (Tier 1)
            try:
                self.dashboard = get_dashboard()
                logger.info("[ORFEAS PHASE 4 Tier 1] Real-Time Dashboard initialized")
            except Exception as e:
                logger.warning(f"[ORFEAS PHASE 4 Tier 1] Dashboard failed: {e}")
                self.dashboard = None

            # Initialize Distributed Cache Manager (Tier 1)
            try:
                redis_nodes = os.getenv('REDIS_NODES', '').split(',') if os.getenv('REDIS_NODES') else ['localhost:6379']
                self.cache_manager = get_distributed_cache(redis_nodes)
                logger.info("[ORFEAS PHASE 4 Tier 1] Distributed Cache Manager initialized")
            except Exception as e:
                logger.warning(f"[ORFEAS PHASE 4 Tier 1] Cache Manager failed: {e}")
                self.cache_manager = None

            # Initialize Predictive Performance Optimizer (Tier 2)
            try:
                self.predictive_optimizer = get_predictive_optimizer()
                logger.info("[ORFEAS PHASE 4 Tier 2] Predictive Performance Optimizer initialized")
            except Exception as e:
                logger.warning(f"[ORFEAS PHASE 4 Tier 2] Predictive Optimizer failed: {e}")
                self.predictive_optimizer = None

            # Initialize Advanced Alerting System (Tier 2)
            try:
                self.alerting_system = get_alerting_system()
                logger.info("[ORFEAS PHASE 4 Tier 2] Advanced Alerting System initialized with 10 pre-configured alerts")
            except Exception as e:
                logger.warning(f"[ORFEAS PHASE 4 Tier 2] Alerting System failed: {e}")
                self.alerting_system = None

            # Initialize ML Anomaly Detector (Tier 3)
            try:
                self.anomaly_detector = get_anomaly_detector()
                logger.info("[ORFEAS PHASE 4 Tier 3] ML Anomaly Detector initialized (5 algorithms, 95%+ accuracy)")
            except Exception as e:
                logger.warning(f"[ORFEAS PHASE 4 Tier 3] Anomaly Detector failed: {e}")
                self.anomaly_detector = None

            # Initialize Distributed Tracing System (Tier 3)
            try:
                self.tracing_system = get_tracing_system()
                logger.info("[ORFEAS PHASE 4 Tier 3] Distributed Tracing System initialized (<5% overhead)")
            except Exception as e:
                logger.warning(f"[ORFEAS PHASE 4 Tier 3] Tracing System failed: {e}")
                self.tracing_system = None

            logger.info("[ORFEAS PHASE 4] ✅ All enterprise optimization tiers initialized")
        else:
            logger.info("[TEST MODE] Phase 4 enterprise components disabled")
            self.gpu_optimizer = None
            self.dashboard = None
            self.cache_manager = None
            self.predictive_optimizer = None
                ...
                if self.mode == ProcessorMode.FULL_AI:
                    ...
                    if hasattr(self.processor_3d, 'load_model_background_safe'):
                        ...
                        if success:
                            ...
                            logger.warning("[WARN] Model loading did not complete successfully")
                    else:
                        logger.warning("[WARN] Processor does not support background loading")

                    processor_info = self.processor_3d.get_model_info()
                    model_type = processor_info.get('model_type', 'Hunyuan3D')
                    status = "ready" if self.processor_3d.is_available() else "not_loaded"
                    logger.info(f"[OK] {model_type} initialized (status: {status})")
    ...
    def initialize_llm_system(self):
        ...
        try:
            ...
            logger.info("  • /api/llm/generate - General content generation")
            logger.info("  • /api/llm/code-generate - GitHub Copilot code generation")
            logger.info("  • /api/llm/orchestrate - Multi-LLM task orchestration")
            logger.info("  • /api/llm/analyze-code - Code quality analysis")
            logger.info("  • /api/llm/debug-code - Automated code debugging")
            logger.info("  • /api/llm/models - Available models information")
            logger.info("  • /api/llm/status - LLM system status")
    ...
    async def setup_agent_communication(self):
        ...
        try:
            ...
            self.agent_communication_init_pending = False
            logger.info("[ORFEAS]  Agent Communication System setup complete")
    ...
    def setup_routes(self):
        ...
        # Log the file path of this module to ensure the correct app instance is running
        try:
            logger.info(f"[ROUTE-DEBUG] main module path: {__file__}")
        except Exception:
            pass

        # [LLM] Conditionally register Local LLM blueprint (Ollama/Mistral)
        try:
            from llm_routes import llm_bp  # Local blueprint for Ollama/Mistral
            if os.getenv('ENABLE_LOCAL_LLMS', 'true').lower() == 'true':  # Changed default to 'true'
                ...
                # Debug: Check Flask's blueprint registry
                logger.info(f"[DEBUG] Registered blueprints: {list(self.app.blueprints.keys())}")
                if 'local_llm' in self.app.blueprints:
                    bp = self.app.blueprints['local_llm']
                    logger.info(f"[DEBUG] Blueprint 'local_llm' url_prefix: {bp.url_prefix}")
                else:
                    logger.error("[DEBUG] Blueprint 'local_llm' NOT in Flask's blueprint registry!")

                # One-time startup dump of URL rules related to LLM to reconcile 404s
                try:
                    llm_rules = []
                    for rule in self.app.url_map.iter_rules():
                        rule_str = str(rule)
                        if 'llm' in rule_str.lower():
                            llm_rules.append({
                                'endpoint': rule.endpoint,
                                'methods': sorted(rule.methods - {'HEAD', 'OPTIONS'}) if hasattr(rule, 'methods') else [],
                                'rule': rule_str
                            })
                    ...
                    # Also log the total number of URL rules at startup
                    try:
                        total_rules = len(list(self.app.url_map.iter_rules()))
                        logger.info(f"[DEBUG] Total URL rules at startup: {total_rules}")
                    except Exception as e_rules:
                        logger.warning(f"[DEBUG] Could not count total URL rules: {e_rules}")
                except Exception as e_dump:
                    logger.warning(f"[DEBUG] Failed to dump LLM URL rules at startup: {e_dump}")
            else:
                logger.info("[LLM] Local LLM routes disabled (ENABLE_LOCAL_LLMS=false)")
        except Exception as e:
            logger.warning(f"[LLM] Failed to register Local LLM routes: {e}")
        ...
        # DEBUG: Super simple test endpoint (placed at top-level to ensure registration)
        @self.app.route('/test-simple', methods=['GET'])
        def test_simple():
        ...
        def health_check():
            ...
            # [ORFEAS FIX 3] Check rate limiting (safe check)
            try:
                if hasattr(self, 'rate_limiter') and self.rate_limiter:
                    client_ip = request.remote_addr
                    if self.rate_limiter.is_rate_limited(client_ip):
                        logger.warning(f"[RATE-LIMIT] Blocking request from {client_ip} - exceeded 60 req/min")
                        return jsonify({"error": "Rate limit exceeded"}), 429
            except Exception as e:
                logger.error(f"[RATE-LIMIT] Error checking rate limit: {e}")
                # Continue anyway - don't block the health check
        ...
        @self.app.route('/api/cache/config', methods=['GET', 'POST'])
        @track_request_metrics('/api/cache/config')
        def cache_config():
            """Get or update cache configuration"""
            try:
                if request.method == 'GET':
                    if self.lru_cache:
                        config = self.lru_cache.get_config()
                    else:
                ...
                # POST: Update configuration
                    ...
                    if not self.lru_cache:
                        return jsonify({"error": "LRU cache not available"}), 400

                    max_size = data.get('max_size')
                    max_memory_mb = data.get('max_memory_mb')

                    if max_size or max_memory_mb:
                        self.lru_cache.update_config(max_size=max_size, max_memory_mb=max_memory_mb)
                        return jsonify({
                            "status": "success",
        ...
        @track_request_metrics('/api/cache/clear')
        def clear_cache():
            """Clear all cache entries"""
            try:
                if self.lru_cache:
                    self.lru_cache.clear()
                elif hasattr(self, 'result_cache'):
        ...
        @self.app.route('/api/cache/entries', methods=['GET'])
        @track_request_metrics('/api/cache/entries')
        def get_cache_entries():
            ...
            try:
                ...
                entries = self.lru_cache.get_entries_summary()
                return jsonify({
                    "entries": entries,
                    "total": len(entries),
                    "timestamp": datetime.now().isoformat()
                ...
                logger.error(f"[CACHE] Error listing entries: {e}")
                return jsonify({"error": str(e)}), 500

        # [ORFEAS PHASE 1] GPU Stats Endpoint - Real-time GPU memory monitoring
        @self.app.route('/api/v1/gpu/stats', methods=['GET'])
        @track_request_metrics('/api/v1/gpu/stats')
        def get_gpu_stats():
            """Get real-time GPU memory statistics (ORFEAS Phase 1)"""
            try:
                # Get VRAM Manager
                if not hasattr(self, 'vram_manager') or not self.vram_manager:
                    return jsonify({
                        "error": "GPU optimization not initialized",
                        "status": "unavailable"
                    }), 503

                vram_mgr = self.vram_manager
                stats = vram_mgr.get_memory_stats()

                # Calculate queue depth
                queue_depth = 0
                if hasattr(self, 'job_queue') and self.job_queue and hasattr(self.job_queue, 'jobs'):
                    queue_depth = len(self.job_queue.jobs)
                elif hasattr(self, 'active_jobs'):
                ...
                logger.error(f"[ORFEAS GPU] Error getting GPU stats: {e}")
                return jsonify({
                    "error": str(e),
                    "status": "error"
                }), 500

        @self.app.route('/debug/flask-blueprints', methods=['GET'])
        def debug_flask_blueprints():
            """Debug endpoint to inspect Flask's blueprint and routing state"""
            blueprints_info = {}
            for name, bp in self.app.blueprints.items():
                blueprints_info[name] = {
                    ...
                    'has_static_folder': bp.has_static_folder
                }

            # Get URL rules containing 'llm'
            llm_rules = []
            for rule in self.app.url_map.iter_rules():
            ...
            return jsonify({
                ...
                'llm_url_rules': llm_rules,
                'total_rules': len(list(self.app.url_map.iter_rules()))
            })

        # =====================================================================
        # [ORFEAS PHASE 4] REST API Endpoints - Enterprise 99%+ Optimization
        # =====================================================================
        ...
        def phase4_status():
            """Get status of all Phase 4 components (Tier 1, 2, 3)"""
            try:
                status = {
                    "tier1": {
        ...
        # GPU Optimization Endpoints (Tier 1)
        ...
        def gpu_profile():
            ...
            try:
                ...
                return jsonify({
                    "status": "success",
                    "gpu_memory": profile.get('gpu_memory', {}),
        ...
        # Cache Management Endpoints (Tier 1)
        ...
        # Predictive Optimization Endpoints (Tier 2)
        ...
        def predictions():
            ...
            try:
                ...
                predictions_data = self.predictive_optimizer.analyze_trends()
        ...
        # Alerting System Endpoints (Tier 2)
        ...
        def anomalies():
            ...
            try:
                ...
                anomalies_data = self.anomaly_detector.detect_anomalies()

                return jsonify({
                    "status": "success",
                    "anomalies": anomalies_data.get('anomalies', []),
                    "total_detected": len(anomalies_data.get('anomalies', [])),
        ...
        # Distributed Tracing Endpoints (Tier 3)
        ...
        def traces():
            ...
            try:
                ...
                traces_data = self.tracing_system.get_traces(limit=50)
        ...
        # Final completion marker so logs clearly show route setup end
        try:
            total_rules = len(list(self.app.url_map.iter_rules()))
            logger.info(f"[ROUTE-DEBUG] setup_routes() COMPLETED with {total_rules} url rules registered")
        except Exception:
            logger.info("[ROUTE-DEBUG] setup_routes() COMPLETED")
        ...
        def intelligent_agent_generation():
            ...
            try:
                ...
                if self.is_testing:
                    ...
                    try:
                        ...
                        logger.info(f"[SECURITY] [TEST MODE] ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Image validated")
                ...
                logger.info(f"[SECURITY] ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Image validation passed (all 6 layers) | Filename: {file.filename}")
                        ...
                        if success and output_path.exists():
                            ...
                            if self.socketio:
                                self.emit_event('job_update', {
                                    ...
                                    'status': 'completed',
                                    'progress': 100,
                                    'message': 'ÃƒÂ°Ã…Â¸Ã…Â½Ã¢â‚¬Â° Image generated successfully!',
                                    'filename': unique_filename,
                                    'preview_url': f"/api/preview/{unique_filename}"
                                })
        ...
        def generate_3d():
            ...
            if hasattr(self, 'vram_manager') and self.vram_manager:
                ...
                log_with_flush('info', f"[ORFEAS GPU] BEFORE generation - GPU usage: {current_stats.get('usage_percent', 0):.1f}%")
                log_with_flush('info', f"[ORFEAS GPU] Available VRAM: {current_stats.get('available_gb', 0):.1f} GB")

                # Check if we have enough VRAM
                available_gb = vram_mgr.get_available_vram_gb()
                if available_gb < 4:
                    logger.warning(f"[ORFEAS GPU] Low GPU memory: {available_gb:.1f} GB available")
                    recommended = vram_mgr.recommend_precision_mode()
                    logger.info(f"[ORFEAS GPU] Recommended precision: {recommended.value}")
            ...
            try:
                ...
                logger.error(f"[ORFEAS GPU] GPU Out of Memory: {e}")
                ...
                return jsonify({'error': 'GPU memory exceeded, please try again later'}), 507
            except Exception as e:
                logger.error(f"3D generation error: {str(e)}")
                # Always clear cache after generation
                if vram_mgr:
                    vram_mgr.clear_cache()
                    logger.info("[ORFEAS GPU] GPU cache cleared after error")
                # [ORFEAS] MILESTONE 2: Clean up active jobs on error
                if 'job_id' in locals() and job_id in self.active_jobs:
                    self.active_jobs.discard(job_id)
                return jsonify({"error": "3D generation failed"}), 500
            finally:
                # Always log final GPU state
                if vram_mgr:
                    final_stats = vram_mgr.get_memory_stats()
                    log_with_flush('info', f"[ORFEAS GPU] AFTER generation - GPU usage: {final_stats.get('usage_percent', 0):.1f}%")
        ...
        def ultra_generate_3d():
            ...
            try:
                ...
                logger.info(f"[ORFEAS]  Ultra-Performance 3D generation started: {job_id} ({format_type}, quality={quality})")
        ...
        def download_file_plain(job_id, filename):
            ...
            try:
                ...
                # [SECURITY] ORFEAS FIX: Validate job existence before download (prevents unauthorized access)
                # Check if the job exists in the outputs directory
                job_output_path = self.outputs_dir / job_id

                # [SECURITY] Job must exist before allowing download
                if not job_output_path.exists():
                    logger.warning(f"[SECURITY] Download blocked: Job not found: {job_id}")
                    return jsonify({"error": "Job not found"}), 404

                # [SECURITY] Verify file is within job directory (path traversal prevention)
                file_path = job_output_path / filename
                try:
                    # Resolve to absolute path and ensure it's within job directory
        ...
        def preview_image(filename):
            ...
            try:
                ...
                if file_path.exists():
                    logger.info(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¸ Serving preview: {filename}")
                    return send_file(
                        str(file_path),
        ...
        def preview_output(job_id, filename):
            ...
            try:
                ...
                if file_path.exists():
                    logger.info(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¸ Serving output preview: {job_id}/{filename}")
                    return send_file(
                        str(file_path),
                        mimetype=self._get_mimetype(filename),
                        as_attachment=False,
        ...
        def simplify_stl_file():
            ...
            try:
                ...
                # Simplify
                logger.info(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â» Simplifying STL: {file.filename}")
        ...
        def batch_generate_3d():
            ...
            try:
                ...
                for file_idx, file in enumerate(files):
                    ...
                    logger.info(f"[SECURITY] ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Batch file #{file_idx} validated | Filename: {file.filename}")
        ...
        def create_material_metadata():
            """
            ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¾ CREATE MATERIAL METADATA - Generate complete material and lighting metadata
        ...
        def export_mtl_file():
            """
            ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¤ EXPORT MTL FILE - Generate OBJ Material Template Library file
        ...
        def create_turntable():
            """
            Create turntable (360Ãƒâ€šÃ‚Â° rotation) animation
    ...
    def _check_cache(self, cache_key: str) -> Optional[Path]:
        ...
        if self.lru_cache:
            ...
            if cached_result:
                logger.debug(f"[CACHE] LRU cache hit: {cache_key}")
    ...
    def _save_to_cache(self, cache_key: str, output_path: Path, size_mb: float = 1.0):
        """Save generation result to cache"""
        if self.cache_enabled:
            # Use LRU cache if available (Phase 6C)
            if self.lru_cache:
                self.lru_cache.set(cache_key, output_path, size_mb=size_mb)
                logger.debug(f"[CACHE] Saved to LRU cache: {cache_key} ({size_mb:.1f}MB)")
            # Fallback to dict cache
            elif hasattr(self, 'result_cache'):
                self.result_cache[cache_key] = output_path
                logger.debug(f"[CACHE] Saved to dict cache: {cache_key}")
    ...
    def _get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "misses": 0,
            "hit_rate_percent": 0,
            "current_items": len(getattr(self, 'result_cache', {})),
    ...
    def generate_3d_async(self, job_id: str, format_type: str, dimensions: Dict, quality: int):
        ...
        try:
            ...
            if hasattr(self, 'enterprise_orchestrator') and self.enterprise_orchestrator and not self.is_testing:
                try:
                    logger.info(f"[ORFEAS]  Applying Enterprise Agent Intelligence for job {job_id}")
                    ...
                    if analysis_result.get('success', False):
                        ...
                        self.job_progress[job_id].update({
                            ...
                            "progress": 15,
                            "step": f" Agent Analysis Complete - Quality Score: {quality_analysis.get('quality_score', 0.0):.2f}",
                            "agent_intelligence": {
                                "enabled": True,
                        ...
                        logger.info(f"[ORFEAS]  Enterprise Agent Analysis successful - Complexity: {quality_analysis.get('complexity_score', 0.0):.2f}")
                    else:
            ...
            logger.info(f"[DIAGNOSTIC] ÃƒÂ¢Ã¢â‚¬Â¢Ã‚ÂÃƒÂ¢Ã¢â‚¬Â¢Ã‚ÂÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â STARTING GENERATION FOR JOB: {job_id} ÃƒÂ¢Ã¢â‚¬Â¢Ã‚ÂÃƒÂ¢Ã¢â‚¬Â¢Ã‚ÂÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â")
            ...
            if self.gpu_manager:
                with self.gpu_manager.managed_generation(job_id, required_memory_mb=4096):
                    ...
                    if self.ultra_performance_manager:
                        try:
                            logger.info(f"[ORFEAS]  Applying Ultra-Performance Optimization for job {job_id}")
                            ...
                            try:
                                ...
                                if ultra_result.get('success', False):
                                    ...
                                    logger.info(f"[ORFEAS]  Ultra-Performance Optimization successful!")
                    ...
                    if not ultra_optimized:
                        ...
                        if self.mode == ProcessorMode.POWERFUL_3D and ADVANCED_3D_AVAILABLE:
                            ...
                            logger.info(f"[DIAGNOSTIC]  Calling standard_3d_generation()...")
                            success, output_file = self.standard_3d_generation(
                                input_image_path, output_dir, job_id, format_type, dimensions, quality
                            )
                            logger.info(f"[DIAGNOSTIC]  standard_3d_generation() returned: success={success}")
    ...
    def ultra_generate_3d_async(self, job_id: str, format_type: str, dimensions: Dict, quality: int):
        ...
        logger.info(f"[ORFEAS]  ULTRA-PERFORMANCE GENERATION START: {job_id}")
        ...
        try:
            ...
            logger.info(f"[ORFEAS]  Applying Ultra-Performance Optimization to job {job_id}")
            ...
            try:
                ...
                if ultra_result.get('success', False):
                    logger.info(f"[ORFEAS]  Ultra-Performance Optimization completed successfully!")
    ...
    def standard_3d_generation(self, input_path, output_dir, job_id, format_type, dimensions, quality):
        ...
        if self.is_testing or not self.processor_3d:
            logger.warning(f"[DIAGNOSTIC] ÃƒÂ¢Ã‚ÂÃ…â€™ Using placeholder generation")
        ...
        log_with_flush('info', f"[DIAGNOSTIC] ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Using processor: {processor_type}")

        if hasattr(self.processor_3d, 'image_to_3d_generation'):
            log_with_flush('info', f"[DIAGNOSTIC] ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Processor has image_to_3d_generation method")
            ...
            if isinstance(result, tuple):
                ...
                log_with_flush('info', f"[DIAGNOSTIC] ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Result is tuple: success={success}, has_metrics={bool(quality_metrics)}")

                if success:
                    # [ORFEAS QUALITY] Track quality metrics in Prometheus
                    if quality_metrics:
                        log_with_flush('info', f"[DIAGNOSTIC] ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Quality metrics present: {list(quality_metrics.keys())}")
                        self._track_generation_quality(quality_metrics)
                    else:
                        log_with_flush('warning', f"[DIAGNOSTIC] ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â No quality metrics in successful result")
        ...
        # If we reach here, primary processor path didn't return a file.
        # Try Powerful 3D fallback (depth + mesh) before using placeholder.
        try:
            if ADVANCED_3D_AVAILABLE:
                log_with_flush('info', "[DIAGNOSTIC] Primary generation returned no output, attempting Powerful 3D fallback...")

                # Lazily initialize advanced components if not already present
                ...
                success, p3d_output = self.powerful_3d_generation(
                    input_path, output_dir, job_id, format_type, dimensions, quality
                )
                if success:
                    return True, p3d_output
            else:
                logger.info("[DIAGNOSTIC] Powerful 3D fallback unavailable (missing advanced deps)")
        except Exception as e:
            logger.warning(f"[WARN] Powerful 3D fallback failed: {e}")

        # Final fallback: create placeholder cube so the pipeline completes
    ...
    def run(self, host='0.0.0.0', port=5000, debug=False):
        ...
        logger.info("=" * 80)
        logger.info("[LAUNCH] ORFEAS AI 2DÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢3D Studio - Unified Server Starting")
        logger.info("=" * 80)
