...
def test_websocket_connection() -> bool:
    ...
    try:
        print(f"\n{BLUE}{'='*78}{RESET}")
        print(f"{MAGENTA} TEST: WEBSOCKET CONNECTION{RESET}")
        print(f"{BLUE}{'='*78}{RESET}")
        ...
        def on_server_connected(data):
            print(f"{CYAN} Server message: {data.get('status', 'Unknown')}{RESET}")
...
def test_batch_ui_features() -> Dict[str, bool]:
    ...
    """
    print(f"\n{BLUE}{'='*78}{RESET}")
    print(f"{MAGENTA} TEST: BATCH STUDIO UI FEATURES{RESET}")
    print(f"{BLUE}{'='*78}{RESET}")
...
def main():
    ...
    if pass_rate == 100:
        ...
        print(f"{GREEN} Phase 2.2 Batch Generation UI is READY!{RESET}")
    elif pass_rate >= 75:
