"""
ORFEAS AI 2D'√ú√≠3D Studio - Performance Profiler
===============================================
ORFEAS AI Project
...
class PerformanceProfiler:
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        if not self.pipeline_profiles:
            recommendations.append("‚ö†Ô∏è No profiling data available - run some pipelines first")
        ...
        if bottlenecks:
            for bottleneck in bottlenecks[:3]:  # Top 3
                ...
                recommendations.append(
                    f"üî¥ {bottleneck['severity'].upper()}: '{stage}' takes {percent:.1f}% "
                    f"({duration:.2f}s) - optimize this stage first"
                )

                # Stage-specific recommendations
                if 'preprocessing' in stage.lower():
                    recommendations.append("   '√ú√≠ Consider caching preprocessed images")
                    recommendations.append("   '√ú√≠ Optimize image resizing with PIL-SIMD or pillow-avif-plugin")
                elif 'shape_generation' in stage.lower() or 'inference' in stage.lower():
                    recommendations.append("   '√ú√≠ Enable mixed precision (FP16) for faster inference")
                    recommendations.append("   '√ú√≠ Reduce inference steps (50 '√ú√≠ 30-40) if quality permits")
                    recommendations.append("   '√ú√≠ Consider model quantization (INT8)")
                elif 'texture' in stage.lower():
                    recommendations.append("   '√ú√≠ Use lower resolution textures for non-critical cases")
                    recommendations.append("   '√ú√≠ Enable texture compression")
                elif 'postprocessing' in stage.lower() or 'export' in stage.lower():
                    recommendations.append("   '√ú√≠ Use binary STL format (faster than ASCII)")
                    recommendations.append("   '√ú√≠ Optimize mesh decimation algorithms")
        else:
            recommendations.append("'√∫√ñ No major bottlenecks detected - pipeline well-balanced")

        # Performance trend recommendations
        avg_duration = summary['avg_duration']
        if avg_duration > 100:
            recommendations.append(f"‚è±Ô∏è Average duration {avg_duration:.1f}s exceeds target (60s)")
            recommendations.append("   '√ú√≠ Target 45% improvement to reach <60s goal")
        elif avg_duration < 60:
            recommendations.append(f"'√∫√ñ Average duration {avg_duration:.1f}s meets performance target!")

        # Variance recommendations
        if summary['max_duration'] > summary['min_duration'] * 2:
            recommendations.append("Ô£ø√º√¨√§ High variance detected - performance inconsistent")
            recommendations.append("   '√ú√≠ Investigate GPU memory contention")
            recommendations.append("   '√ú√≠ Check for I/O bottlenecks")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("'√∫√ñ Performance Profiler test complete!")
    print("=" * 80)
