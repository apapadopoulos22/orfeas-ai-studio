...
# Usage: .\deploy_linux.ps1 -RemoteHost "user@linux-server" -Action Initialize
...
docker run --rm --gpus all nvidia/cuda:12.1.0-runtime-ubuntu22.04 nvidia-smi > /dev/null 2>&1 || { echo "✗ NVIDIA Container Toolkit not configured"; exit 1; }

echo "✓ All prerequisites met"
"@

    Invoke-RemoteCommand -Host $RemoteHost -Command "bash -c '$($prereqs)'"
    Write-Success "Prerequisites verified"

    # 2. Create directories
    Write-Info "Creating directories..."
    Invoke-RemoteCommand -Host $RemoteHost -Command "mkdir -p $OurfeasDir && cd $OurfeasDir && mkdir -p models outputs uploads temp logs"
    Write-Success "Directories created"

    # 3. Download repository (assuming SSH access to GitHub)
    Write-Info "Cloning ORFEAS repository..."
    Invoke-RemoteCommand -Host $RemoteHost -Command "cd $(Split-Path $OurfeasDir) && git clone <YOUR_REPO_URL> $(Split-Path -Leaf $OurfeasDir) 2>/dev/null || echo 'Repository already exists'"
...
"@
    ...
    Invoke-RemoteCommand -Host $RemoteHost -Command "cat > $env_file << 'ENVEOF'`n$env_content`nENVEOF"
    Write-Success ".env created"

    # 5. Build image
    Write-Info "Building Docker image (this may take 5-10 minutes)..."
    Invoke-RemoteCommand -Host $RemoteHost -Command "cd $OurfeasDir && docker-compose build --no-cache"
    Write-Success "Docker image built"

    # 6. Start services
    Write-Info "Starting services..."
    Invoke-RemoteCommand -Host $RemoteHost -Command "cd $OurfeasDir && docker-compose up -d"
    ...
    for ($i = 0; $i -lt 120; $i++) {
        try {
            $response = curl -s "http://$($RemoteHost):5000/api/health" 2>/dev/null
...
# ACTION: STOP
# ============================================================================
function Stop-ORFEAS {
    Write-Info "Stopping ORFEAS on $RemoteHost..."
    Invoke-RemoteCommand -Host $RemoteHost -Command "cd $OurfeasDir && docker-compose down"
...
# ACTION: STATUS
# ============================================================================
function Status-ORFEAS {
    Write-Info "Checking status on $RemoteHost..."

    # Health check
    try {
        $response = curl -s "http://$($RemoteHost):5000/api/health"
        if ($response) {
            $health = $response | ConvertFrom-Json
            Write-Success "Backend: Online"
            Write-Info "  Status: $($health.status)"
            if ($health.model) { Write-Info "  Model: $($health.model)" }
            if ($health.gpu_memory) { Write-Info "  GPU Memory: $($health.gpu_memory.used_gb) GB / $($health.gpu_memory.total_gb) GB" }
        }
        else {
            Write-Warning "Backend: Offline or unresponsive"
        }
    }
    catch {
        Write-Warning "Backend: Cannot connect"
    }

    # Docker status
    Write-Info "Docker services:"
    Invoke-RemoteCommand -Host $RemoteHost -Command "cd $OurfeasDir && docker-compose ps"
}

# ============================================================================
# ACTION: TEST
# ============================================================================
function Test-ORFEAS {
    ...
    if (-not (Test-Path "backend/test_images/test_image.png")) {
        Write-Error "Test image not found at backend/test_images/test_image.png"
        return
    }
...
"@
    ...
    Invoke-RemoteCommand -Host $RemoteHost -Command "cd $OurfeasDir && python3 -c '$testScript'"
}
