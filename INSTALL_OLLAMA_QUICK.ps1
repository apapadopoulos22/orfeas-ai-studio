...
if ($ollamaInstalled) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "`n⚠️  NOTE: Ollama needs to be restarted to work properly"
...
try {
    ...
    Write-Host "⚠️  Model test failed" -ForegroundColor Yellow
...
"@
    ...
    Write-Host "⚠️  .env not found, creating new one..." -ForegroundColor Yellow
    $envConfig = @"
...
Write-Host "`n📊 What you just set up:" -ForegroundColor Yellow
Write-Host "   ✅ Ollama local LLM server (localhost:11434)" -ForegroundColor Green
Write-Host "   ✅ Mistral model (4.1GB, <100ms latency)" -ForegroundColor Green
Write-Host "   ✅ ORFEAS configured for local processing" -ForegroundColor Green

Write-Host "`n🚀 Performance improvement:" -ForegroundColor Yellow
...
Write-Host "   Savings: \$1,000-10,000 per year 💰" -ForegroundColor Green
...
Write-Host "`n💡 Other available models:" -ForegroundColor Yellow
...
Write-Host "`n📚 Documentation: LOCAL_AI_SETUP_GUIDE.md" -ForegroundColor Cyan
Write-Host "`n" + "="*70 -ForegroundColor Cyan
